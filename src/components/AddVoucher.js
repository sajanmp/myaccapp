...
function AddVoucher() {
    ...
    const getDbAccHeads = async (ser) => {
        ...
        if (ser === "CR") {
            q = query(accHeadsCollectionRef, where("type", "==", "Cash"), where("active", "==", true))
        }

        if (ser === "BR") {
            q = query(accHeadsCollectionRef, where("type", "==", "Bank"), where("active", "==", true))
        }

        if (ser === "CV") {
            q = query(accHeadsCollectionRef, where("type", "in", ["Cash", "Bank"]))
        }

        if (ser === "JV" || ser === "CP" || ser === "BP") {
            q = query(accHeadsCollectionRef, where("type", "not-in", ["Cash", "Bank"]))
        }
